...
class Component_supercritical_deepzoom extends Component {
      ...
      if (file_exists($response["xmlpath"])) {
        ...
        if (!empty($img)) {
          ...
          $response["imgdata"] = array("size" => array((int)$img->Size["Width"], (int)$img->Size["Height"]),
                                   ...
                                   "url" => (!empty($img["Url"]) ? (string)$img["Url"] : sprintf("http://%s/images/components/supercritical/deepzoom/%s_files/{level}/{column}_{row}.%s", $defaultdomain, $response["imgname"], $response["fileext"])),
                                   );
        }
      }
    }
