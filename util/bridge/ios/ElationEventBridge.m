...
- (BOOL) webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
	...
	if ([request.URL.scheme isEqualToString:kElationEventBridgeNotificationName])
    {
        ...
        NSData * eventData = [eventDataStr dataUsingEncoding:NSUTF8StringEncoding];
	    ...
	    if (![eventDataStr isEqualToString:@"/"])
        {
            ...
            NSDictionary *jsonData = [NSJSONSerialization JSONObjectWithData:eventData options:0 error:&jsonError];
        	if (jsonData != nil) {
