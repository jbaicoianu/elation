...
<select{if !empty($selectname)} name="{$selectname}"{/if}{if !empty($id)} id="{$id}"{/if}{if !empty($class)} class="{$class}"{/if}{if !empty($autosubmit)} onchange="this.form.submit()"{/if} elation:component="ui.select"{if isset($tabindex)} tabindex="{$tabindex}"{/if}>
