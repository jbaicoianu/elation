  ...
  function App($rootdir, $args) {
    ...
    if ($this->initialized()) {
      try {
        ...
        $this->cfg->GetConfig($this->cobrand, true, $this->cfg->activerole);
...
//    print "$class <br />";
  ...
  function ApplyConfigOverrides() {
    ...
    if(!empty($this->request["args"]["cobrandoverride"])) {
      $included_config =& $this->cfg->GetConfig($this->request["args"]["cobrandoverride"], false, $this->cfg->activerole);
      if (!empty($included_config))
        ConfigManager::merge($included_config);
    }
    $rolename = any($this->request["args"]["_role"], $this->cfg->activerole);
    ...
    if ($this->request["ssl"]) {
      $included_config = $this->cfg->GetConfig("classes.secure", false, $this->cfg->activerole);
      if (!empty($included_config))
        ConfigManager::merge($included_config);
    }
    ...
    if(!empty($this->request["browser"])) {
      ...
      if (!empty($included_config["include"])) { // These includes sure do get hairy.  This allows for browsers.*.options.include to call in different classes
        ...
        foreach ($includes as $include) {
          $subincluded_config =& $this->cfg->GetConfig($include, false, $this->cfg->activerole);
    ...
    if ($this->debug) {
      $subincluded_config =& $this->cfg->GetConfig("classes.debug", false, $this->cfg->activerole);
      if (!empty($subincluded_config))
        ConfigManager::merge($subincluded_config);
    }
  }
